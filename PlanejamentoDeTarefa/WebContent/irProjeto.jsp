		...
		<main id="main">
			...
			<div style="text-align: center; margin-left: 30px;">
				...
				<fieldset
					...
					<fieldset style="text-align: center;margin-left:25% ;border-radius:40px; width:40%;">
					...
					%>
						<% //VERIFICA QUANTO TEMPO FALTA PARA *****COMEÇAR*******
							//VAI MUDANDO DE COR CONFORME O PRAZO PASSA
						if (diaIni>7){
							%>
							<label style="color: green"> <%out.println(diaIni);%> dias para começar.</label>	
						<%	
						}else if(diaIni>3){
							%>
							<label style="color: brown"><%out.println(diaIni);%> dias para começar.</label>	
							<%
						}else if(diaIni==2){
							%>
							<label style="color: orange;"><%out.println(diaIni);%> dias para começar.</label>	
							<%
						}else if(diaIni==1){
							%>
							<label style="color: red"><%out.println(diaIni);%> dia para começar. </label>	
							<%
						}else{
							...
							if(diaIni == 1){
							%>
							<label style="color: red">Começou a <%out.println(diaIni);%> dia</label>
							<%
							}else{
							%>
							<label style="color: red">Começou a <%out.println(diaIni);%> dias</label>	
							<%
							}
						}
						%>
						...
						<%	
							...
							%>
							<label style="color: red">Começou a <%out.println(diaFim);%> dias</label>	
							<%
							}
						}
						%>
					...
					<br>
						<%
							out.println("<label > Id: " + request.getAttribute("idProjeto") + "</label>");
						%><br>
							<%
							out.println("<label > Identificação: " + request.getAttribute("identificacao") + "</label>");
						%><br>
							<%
