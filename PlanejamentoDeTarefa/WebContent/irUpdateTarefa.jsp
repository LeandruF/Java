		...
		<main id="main">
			...
			PEGAR CPF DE TODOS AS PESSOAS E COLOCAR NO CPF_TITULAR CASO QUEIRA TROCAR O TITULAR
			 -->
						<label>Id:</label>
						<br>
						<input name="id"id="id"  type="text" size = "30" maxlength="100" readonly="readonly" value="<%=request.getAttribute("idTarefa")%>"/><br>
						<label > Nome:</label> 
						<br>
						...
						<label > Projeto:<%=request.getAttribute("idProjeto")%></label> <br>
						<select id="idProjeto" name="idProjeto" style="
						margin-top:10px; 
						width: 45%;
						border-radius: 20px;
						line-height: 40px;
						...
						<option name="null" value="null"></option>
							<%@ page import="controller.Projeto "%>
						<% List<Projeto> listaProjetos = new ArrayList<Projeto>();
						Projeto pro = new Projeto();
						listaProjetos = pro.listarProjeto();
						for(int i = 0;i<listaProjetos.size();i++){
							%>
							<option value = "<%= listaProjetos.get(i).getId()%>"name="<%= listaProjetos.get(i).getId()%>"><%=listaProjetos.get(i).getNome()%></option>
						...
						<input name="nomeEquipe"id="nomeEquipe"  type="text" size = "30" maxlength="100" value="<%=request.getAttribute("nomeEquipe")%>"/><br>	
						<br>
						<label > Status: </label><label name="stat"><%=request.getAttribute("status")%></label> <br>
				  ...
				  <input type="radio" id="cancelado" name="status" value="cancelado"><label for="cancelado">Cancelado</label>
					...
					<input style="width: 200px" type="submit" value="Confirmar " formaction="servletConfirmaUpdateTarefa" formmethod="post"/>
							</td>
							<td style="width:250px">
